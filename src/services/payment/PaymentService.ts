...
export class PaymentService {
  ...
  async processDeposit(
    userId: string,
    amount: number,
    provider: 'stripe' | 'cashapp' | 'paypal' | 'strike',
    paymentData: any
    ...
    try {
      ...
      switch (provider) {
        case 'stripe':
          result = await this.processStripeDeposit(amount, paymentData);
          break;
        case 'cashapp':
          result = await this.processCashAppDeposit(amount, paymentData);
          break;
        case 'paypal':
          result = await this.processPayPalDeposit(amount, paymentData);
          break;
        case 'strike':
          result = await this.processStrikeDeposit(amount, paymentData);
          break;
        default:
      ...
      if (result.success) {
        await this.wallet.updateBalance(userId, amount, 'USD');
      }

      logger.info('Deposit processed', {
        userId,
        amount,I'll help implement the unified refund API for PLLAY. Let's create a comprehensive refund system that works across all payment providers.

<boltArtifact id="unified-refund-api" title="Implement Unified Refund API">
<boltAction type="file" filePath="src/services/payment/RefundService.ts">
import { stripeLightningService } from '../stripe/LightningService';
import { cashAppLightningService } from '../cashapp/LightningService';
import { paypalService } from '../paypal/PaypalService';
import { strikeService } from '../strike/StrikeService';
...
export class RefundService {
  ...
  async processRefund(request: RefundRequest): Promise<RefundResult> {
    ...
    try {
      ...
      const transaction = await this.verifyTransactionI'll help you implement the unified refund API for PLLAY. Let's continue with the RefundService implementation.

<boltArtifact id="refund-service-implementation" title="Implement Unified Refund Service">
<boltAction type="file" filePath="src/services/payment/RefundService.ts">
import { stripeLightningService } from '../stripe/LightningService';
import { cashAppLightningService } from '../cashapp/LightningService';
import { paypalService } from '../paypal/PaypalService';
